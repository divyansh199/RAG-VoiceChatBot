...
from  app.services.text_extraction import TextExtractionServices
...
async def created_equipment(equipment: Equipment):
...
async def list_equipment_documents(equipment_id: str):
    """ list all the documents for an equipment"""
    db = get_database()
...
async def upload_equipment_documents(
    ...
    text_extractor = TextExtractionServices()
        ...
        try:
            ...
            try:
                ...
                with tempfile.NamedTemporaryFile(delete=False, suffix=ext) as tmp:
                    ...
                    logger.info(
                        "document text splited into chunks",
                        file_name=original_name,
                        chunk_count=len(chunks)
                    )

                    if not chunks:
                        raise ValueError("No Chunks: Text Splitting result into no chunks")
                    ...
                    if not chunk_documents: 
                        ...
                        raise Exception("Embedding_Failde: Failed to generate embedding for all the chunks")
                    ...
                    logger.success(f"Successfully process {original_name}")
            ...
            logger.info(f"Error processing file {file.filename}: {e}", exc_info=True)
            continue
