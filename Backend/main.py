...
app = FastAPI(
    ...
    description="Industrial Voice Bot with RAG",
    version="0.1.0",
    lifespan=lifespan
)
...
if allowed_origins_env:
    origins = [origin.strip() for origins in allowed_origins_env.split(",")]
else:
    origins = [
        "http://localhost:5173",
        "http://localhost:3000",
...
@app.get('/')
def read_root():
    return{"message": "Industrial api is running", "version": "0.1.0"}
